...
void popup_window_create_ui() {
	...
	qui_widget* layout = qui_create_vertical_layout(popup_ui);
	{
		qui_widget* top_container = qui_create_flex_container(layout, 1);
		{
			...
			qui_widget* text_layout = qui_create_vertical_layout(top_container);
			{
				qui_create_label(text_layout, "Test label");
			}
		}
		qui_widget* bottom_container = qui_create_fixed_container(layout, ITEMBAR_H);
		{
			...
			{
				qui_create_button(button_layout, "Accept");
				qui_create_button(button_layout, "Cancel");
			}		
		}
	}
}
