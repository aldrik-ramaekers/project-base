...
static void gl_render_image_quad_partial_tint(image *image, s32 x1, s32 y1, s32 x2, s32 y2, s32 x3, s32 y3, s32 x4, s32 y4, vec2f tl, vec2f tr, vec2f bl, vec2f br, color tint)
{
    ...
    if (image->loaded)
    {
        ...
        int width = x4 - x1;
		int height = y3 - y4;
		float origx = x1 + (width / 2.0f);
		float origy = y1 + (height / 2.0f);

        IMP_glTranslatef(origx, origy, 0.0f);
		IMP_glRotatef(global_rotation*(180/M_PI), 0.0f, 0.0f, 1.0f);
		
        IMP_glBegin(GL_QUADS);
		IMP_glTexCoord2f(tl.x, tl.y);
        IMP_glVertex3i(-width/2, -height/2, gl_render_depth);
        IMP_glTexCoord2f(tr.x, tr.y);
        IMP_glVertex3i(width/2, -height/2, gl_render_depth);
        IMP_glTexCoord2f(br.x, br.y);
        IMP_glVertex3i(width/2, height/2, gl_render_depth);
        IMP_glTexCoord2f(bl.x, bl.y);
        IMP_glVertex3i(-width/2, height/2, gl_render_depth);
        IMP_glEnd();
...
static void gl_render_image_quad_partial(image *image, s32 x1, s32 y1, s32 x2, s32 y2, s32 x3, s32 y3, s32 x4, s32 y4, vec2f tl, vec2f tr, vec2f bl, vec2f br)
{
	log_assert(image, "Image cannot be null");
...
static void gl_render_image_quad(image *image, s32 x1, s32 y1, s32 x2, s32 y2, s32 x3, s32 y3, s32 x4, s32 y4)
{
    log_assert(image, "Image cannot be null");
...
static void gl_render_image_quad_tint(image *image, s32 x1, s32 y1, s32 x2, s32 y2, s32 x3, s32 y3, s32 x4, s32 y4, color tint)
{
    log_assert(image, "Image cannot be null");
...
static void gl_render_image(image *image, s32 x, s32 y, s32 width, s32 height)
{
    ...
    if (image->loaded)
    {
        ...
        float origx = x + (width / 2.0f);
		float origy = y + (height / 2.0f);
...
render_driver render_gl_driver =
{
	...
	gl_render_image_quad,
	gl_render_image_quad_tint,
	gl_render_image_quad_partial,
	gl_render_image_quad_partial_tint,
